...
Scrape adidas.com search (PLP) and product (PDP) pages conversationally from any MCP-capable AI agent or client using the
...
```
Using the scrapeless tools, open https://www.adidas.com/us/samba-og-shoes/B75806.html and return
the Product fields from ../../DATA_MODEL.md (id, url, name, brand, description, price,
priceCurrency, availability, images, rating, reviewCount, category, color). The product data
lives in the page's application/ld+json Product block.
```

## 4. Scrape a search page

```
Using the scrapeless tools, open https://www.adidas.com/us/men-shoes and return each product card
as { id, url, name, image, price, priceCurrency }. The cards come from the application/ld+json
ItemList block (or the [data-testid="plp-product-card"] cards as a fallback).
```

## 5. Output shape

The MCP surface emits whatever shape you ask for. To stay aligned with the `cli/`, `nodejs/`, and
`python/` surfaces, ask for the `SearchResult` and `Product` fields documented in
...
- Set `proxyCountry` to `US` at session creation for USD pricing and US inventory — adidas localizes
  price, currency, and availability by region; per-call `proxyCountry` may be ignored.
