# Redbubble — MCP surface

Scrape Redbubble product and search results pages conversationally from any MCP-capable AI agent or client using the
...
```
Using the scrapeless tools, open https://www.redbubble.com/i/sticker/Everything-s-good-cat-by-blah707/43977882/7sgk
and return the Product fields from ../../DATA_MODEL.md (id, name, medium,
artist, price, priceCurrency, availability, images, rating, reviewCount).
```

## 4. Scrape a search results page

```
Open https://www.redbubble.com/shop/cat with the scrapeless browser and return
each card as a SearchResult { id, url, name, artist, medium, image, price }.
```
...
- Redbubble pages are Next.js-rendered — the `__NEXT_DATA__` script is the source of truth, with
  JSON-LD `Product` on the PDP as a fallback.
