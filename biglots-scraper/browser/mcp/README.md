# Big Lots — MCP surface

Scrape Big Lots category and product pages conversationally from any MCP-capable AI agent or client — no code. The LLM drives a Scrapeless cloud browser through MCP tools
...
Once the server is connected, ask in plain language. Start with a category (search) page:

```
Using the scrapeless tools, open https://biglots.com/product-category/pets/ and return
each result card as { id, url, name, image, price, priceCurrency, category }.
```

## 4. Scrape a product page

```
Open https://biglots.com/product/serta-jumbo-bed-pillow/ with the scrapeless browser
and extract the Product fields from ../../DATA_MODEL.md (id, name, description, price,
priceCurrency, availability, images, categories, sellerName).
```
