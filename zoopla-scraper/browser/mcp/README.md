...
Scrape Zoopla property detail pages conversationally from any MCP-capable AI agent or client — no code. The LLM drives a Scrapeless cloud browser through MCP tools and you
...
https://www.zoopla.co.uk/new-homes/details/70337559/ and extract the Property
fields from ../../DATA_MODEL.md (id, url, title, address, price, gallery,
numOfBedrooms, propertyInfo, coordinates, nearby, agent, ...) from the rendered DOM.
```
...
- Zoopla is geo-locked + Akamai-protected — create the session with a GB proxy region; per-call
  `proxyCountry` may be ignored, so set the country at session creation.
- Property pages have no clean JSON cache — ask the model to read the rendered DOM (the `key-info`,
  `local-area`, and `Contact agent` sections carry the structured fields).
