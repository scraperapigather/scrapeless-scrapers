...
> **Status: Preview.** Google Gemini requires a signed-in Google account, so this scraper needs an authenticated Scrapeless session profile to return answers. Selectors are illustrative and the committed `results/*.json` are schema-valid samples **pending live verification**. The shape in [`DATA_MODEL.md`](DATA_MODEL.md) is the stable contract.

[gemini.google.com](https://gemini.google.com/) scraper powered by [Scrapeless](https://www.scrapeless.com/). Every surface drives a Scrapeless cloud [Scraping Browser](https://www.scrapeless.com/en/product/scraping-browser) and emits identical JSON shapes — see [`DATA_MODEL.md`](DATA_MODEL.md).

## Authentication

Gemini's web app requires a signed-in Google account. The scraper reuses a **Scrapeless session profile** that already carries the Google login cookies — set `SCRAPELESS_PROFILE_ID` to a profile you have signed into once. Without a signed-in profile the run lands on the Google sign-in page and returns an empty answer. See the auth section in [`DATA_MODEL.md`](DATA_MODEL.md).
...
The same data is also available through the Scrapeless **Scraper API** — one HTTP request, no browser to drive. This collection maps to the `scraper.gemini` actor:
...
resp = requests.post(
    ...
    json={"actor": "scraper.gemini", "input": {"action": "<see API docs>"}},
)
resp.raise_for_status()
print(resp.json())
```

The exact `input.action` and field names for `scraper.gemini` are listed in the [Scrapeless API docs](https://apidocs.scrapeless.com). Swap the `input` block for the actor's documented parameters.
