...
Scrape Zara product (PDP) and listing (PLP) pages conversationally from any MCP-capable AI agent or client — no code. The LLM drives a Scrapeless cloud browser through MCP
...
https://www.zara.com/us/en/zw-collection-fitted-plaid-jacket-p02784381.html and
extract the Product fields from ../../DATA_MODEL.md (id, name, brand, price,
priceCurrency, availability, images, color, category) from the page's
...
```
Open https://www.zara.com/us/en/woman-blazers-l1055.html with the scrapeless
browser and return each PLP card as { id, url, name, image, price, priceCurrency }.
```
...
- Zara.com is Cloudflare-fronted — Scrapeless's residential fingerprinting handles the JS challenge
  transparently.
- Zara's ItemList JSON-LD omits per-item URLs; ask the model to fall back to DOM anchors matching
  `-p<digits>.html` when the URL is missing.
