...
Scrape Copilot answers conversationally from any MCP-capable AI agent or client using the [Scrapeless MCP server](https://github.com/scrapeless-ai/scrapeless-mcp-server).
...
Once the server is connected, ask in plain language. Copilot has no `?q=` deep link, so the model
opens the homepage, types the prompt into the composer, lets it stream, then reads it back:

```
Using the scrapeless tools, open https://copilot.microsoft.com/, type
"top 3 smartphones in 2025, compare pricing across US marketplaces" into the chat composer
(textarea, [contenteditable='true'], or [role='textbox']) and press Enter, wait for the assistant
answer bubble ([data-content='ai-message'], [data-testid='message-text'], or div[class*='prose'])
to finish streaming, and return one Search object from ../../DATA_MODEL.md:
{ query, url, answer_text, citations }.
```

`citations` should be the deduped outbound `<a href="http...">` links that are not
Microsoft-internal, each as `{ url, domain, title }`. The `query` is the prompt that was submitted
(Copilot has no on-page question heading), and `answer_text` is the last assistant message bubble.
