# Leboncoin — MCP surface

Scrape Leboncoin search and ad pages conversationally from any MCP-capable AI agent or client — no code, the LLM drives a Scrapeless cloud browser through MCP tools.
...
```
Using the scrapeless tools, open https://www.leboncoin.fr/recherche?text=coffe
and return each ad from the __NEXT_DATA__ blob (props.pageProps.searchData.ads)
as { list_id, subject, url, category_id, category_name, price }.
```

## 4. Scrape an ad page

The full ad detail lives in `props.pageProps.ad` inside the same `__NEXT_DATA__` blob:

```
Open a Leboncoin ad URL with the scrapeless browser and extract the full Ad
fields from ../../DATA_MODEL.md (list_id, subject, body, price, images,
...
`python/` surfaces, ask for the `SearchAd` and `Ad` fields documented in
