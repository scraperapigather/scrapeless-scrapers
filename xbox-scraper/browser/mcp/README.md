...
Scrape Xbox store product and discovery pages conversationally from any MCP-capable AI agent or client — no code. The LLM drives a Scrapeless cloud browser through MCP tools
...
https://www.xbox.com/en-us/games/store/marvel-rivals/9N8PMW7QMD3D and return
the Product fields from ../../DATA_MODEL.md (id, name, price, genre, platforms,
ratingValue, videos, ...) from the page's ld+json @graph.
```

## 4. Scrape a search page

```
Open https://www.xbox.com/en-us/games/all-games with the scrapeless browser and
return each game tile as { id, slug, name, url, image, badge }.
```
...
- Xbox store pages ship the full structured payload in a single `application/ld+json` script — ask
  the model to parse the `@graph` `Product` node.
