# Trip.com — MCP surface

Scrape Trip.com conversationally from any MCP-capable AI agent or client — no
code. The LLM drives a Scrapeless cloud browser through MCP tools and you ask for the fields
...
## 3. Scrape a hotel list page

Once the server is connected, ask in plain language. Start with a city hotel list:

```
Using the scrapeless tools, open https://www.trip.com/hotels/list?city=53 and
return each hotel card as { id, name, url, score, reviewWord, reviewCount,
price, totalPrice, tags, location, image }.
```

## 4. Scrape a hotel detail page

```
Open https://www.trip.com/hotels/detail/?hotelId=106534464 with the scrapeless
browser and extract the Hotel fields from ../../DATA_MODEL.md (id, name, address,
score, reviewCount, description, amenities, images, price).
```
