...
Scrape GOAT product and search pages conversationally from any MCP-capable AI agent or client — no code. The [Scrapeless MCP server](https://github.com/scrapeless-ai/scrapeless-mcp-server)
...
and return the Product object embedded in __NEXT_DATA__ (id, name, brandName,
color, sku, slug, sizeRange, offers, ...).
```

## 4. Scrape a search page

```
Open https://www.goat.com/search?query=pumar%20dark with the scrapeless browser
and extract each SearchResult from ../../DATA_MODEL.md (id, slug, title,
pictureUrl, brandName, variantsList, ...).
```
