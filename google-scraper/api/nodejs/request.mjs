...
export async function scrapeGoogle(query) {
  const resp = await fetch(ENDPOINT, {
    ...
    body: JSON.stringify({ actor: "scraper.google.search", input: { q: query } }),
  });
  if (!resp.ok) throw new Error(`HTTP ${resp.status}: ${await resp.text()}`);
  return resp.json();
}

const data = await scrapeGoogle("web scraping");
// google.search flattens the parsed SERP result at the top level;
